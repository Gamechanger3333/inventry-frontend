...
import {
  ...
  useListSuppliers, useListWarehouses, useListProducts,
  type PurchaseOrder, type PurchaseOrderInput,
...
const statusColor = (s: string) =>
  s === "received" ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-100" :
  s === "ordered" ? "bg-blue-100 text-blue-700 hover:bg-blue-100" :
  s === "draft" ? "" : "bg-rose-100 text-rose-700 hover:bg-rose-100";
...
export default function PurchasesPage() {
  ...
  const [form, setForm] = useState<PurchaseOrderInput>({ supplierId: 0, warehouseId: 0, items: [], notes: "" });
  const [newItem, setNewItem] = useState({ productId: 0, quantity: 1, unitCost: 0 });

  const { data: orders = [], isLoading } = useListPurchaseOrders({ search: search || undefined, status: statusFilter || undefined });
  const { data: suppliers = [] } = useListSuppliers();
  const { data: warehouses = [] } = useListWarehouses();
  const { data: products = [] } = useListProducts();

  const invalidate = () => qc.invalidateQueries({ queryKey: getListPurchaseOrdersQueryKey() });
  const createMut = useCreatePurchaseOrder({ mutation: { onSuccess: () => { invalidate(); setOpen(false); toast({ title: "PO created" }); } } });
  const updateMut = useUpdatePurchaseOrder({ mutation: { onSuccess: () => { invalidate(); setOpen(false); toast({ title: "PO updated" }); } } });
  const deleteMut = useDeletePurchaseOrder({ mutation: { onSuccess: () => { invalidate(); toast({ title: "PO deleted" }); } } });

  const openCreate = () => { setEditing(null); setForm({ supplierId: 0, warehouseId: 0, items: [], notes: "" }); setOpen(true); };

  const addItem = () => {
    if (!newItem.productId || !newItem.quantity) return;
    setForm({ ...form, items: [...form.items, newItem] });
    setNewItem({ productId: 0, quantity: 1, unitCost: 0 });
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editing) updateMut.mutate({ id: editing.id, purchaseOrderUpdate: { status: editing.status } });
    else createMut.mutate({ purchaseOrderInput: form });
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage supplier procurement</p>
        </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">
                <Truck className="w-8 h-8 mx-auto mb-2 opacity-40" />No purchase orders found
              </TableCell></TableRow>
            ) : orders.map((o) => (
              <TableRow key={o.id}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{editing ? `PO ${editing.orderNumber}` : "New Purchase Order"}</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Input type="number" min="0" step="0.01" value={newItem.unitCost} onChange={(e) => setNewItem({ ...newItem, unitCost: Number(e.target.value) })} className="w-24" placeholder="Cost" />
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={createMut.isPending || updateMut.isPending}>{editing ? "Update" : "Create PO"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
